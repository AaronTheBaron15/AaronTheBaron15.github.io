...
bool PlaceQueen(int column) {
	if (column == NUMBER_COLS) { // Base Condition
		...
		for (int row = 0; row < RowPlacement.size(); ++row) {
			...
			// FIXED: Assign column to Row[row]
			RowPlacement[row] = column;

			// FIXED: Recursively call PlaceQueen to place a queen in the next column.
			...
			// FIXED: Unassign the assignment of the column's queen to this row, and try the next one
