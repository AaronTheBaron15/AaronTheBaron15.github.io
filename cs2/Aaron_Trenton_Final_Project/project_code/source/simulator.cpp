...
int main() {
    ...
    //Calls applyToVector function to put data from four files into four vectors, one for bases, one for flavors, one for toppings, and one for customer names
    ...
    Shop creamStore = Shop(flavorServings, toppingServings, baseServings, allFlavors, allToppings, allBases, allNames, 1000); //Makes ice cream shop with vectors of bases, flavors, and toppings, and 1000 dollars
    
    cout << "Shop has $" << creamStore.getMoney() << endl;

   
    int numDays = (rand()%5)+10; //Shop will run for a random number of days
    int numCusts = 0; //Counter for served customers

    for(int i = 1; i <= numDays; i++){ //i represents day of shop operation
        for(int h = 1; h <= 5; h++){ //h represents work hour of day
            cout << "Hour " << h << ": " << endl;
            cout << endl;
            for(int c = 1; c <= (rand()%7)+5; c++){ //c represents customer that enters during hour
                creamStore.addCustomer(); //Random number of customer enter each hour
                numCusts++; //Number of customers served increments after each customer
                cout << "Customer enters!" << endl;
                cout << creamStore.getQ().back() << endl; //Prints information about each customer after he or she enters
            }

            while(!creamStore.getQ().empty()){

                cout << creamStore.getQ().front().getName() << " ordered " << creamStore.makeOrder(creamStore.getQ().front()) << endl; //Prints ice cream ordered by customer
                cout << creamStore.getQ().front().getName() << " leaves" << endl;
                creamStore.removeCustomer(); //Customer leaves line (queue)
                cout << endl;
            }
        }

        cout << "Money after day " << i << ": $" << creamStore.getMoney() << endl; //Prints total amount of money shop has after each day
        cout << endl;
    }
